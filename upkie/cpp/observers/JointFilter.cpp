...
JointFilter::JointFilter(const Parameters& params) : params_(params) {
  ...
  joints_.insert(joints_.end(), params_.upper_leg_joints.begin(),
                 params_.upper_leg_joints.end());
  ...
  for (const auto& joint : joints_) {
    for (const auto& variable : params_.variables) {
      if (!observation("servo")(joint).has(variable)) {
        spdlog::warn("[JointFilter] Missing variable {} for joint {}", variable,
                     joint);
        return;
      }
      // read variable
